...
bool DeepType::saveModel(const char *out_file) {
  LOCK
...
void DeepType::predict(const int *ids, size_t idCount, bool partialSequence, int topN, bool save_logits) {
  ...
  struct PredictTimeLogger {
      ...
      inline PredictTimeLogger(int &_cost, size_t &_steps) : cost(_cost), steps(_steps) {
      ...
      inline ~PredictTimeLogger() {
        ...
        LOGD("predict %d step, cost %dms", static_cast<int>(steps), cost);
      }
  ...
  outputs.reserve(mStates.size() + 3);
  ...
  if (topN > 0)
    ...
    mPredicted.clear();
  LOGD("predict result: %d %f", mPredicted[0].first, mPredicted[0].second);
...
void DeepType::train_online(const int *ids, size_t wordCount, size_t letterCount, int id_out, float learning_rate, bool infer_reuse) {
  ...
  struct TrainTimeLogger {
      ...
      inline TrainTimeLogger(int &_cost) : cost(_cost) {
        clock_gettime(CLOCK_REALTIME, &begin);
      }
      ...
      inline ~TrainTimeLogger() {
        clock_gettime(CLOCK_REALTIME, &end);
        cost = 1000.0 * (end.tv_sec - begin.tv_sec) + (double) (end.tv_nsec - begin.tv_nsec) / 1e6;
        LOGD("train cost %dms", cost);
      }
  } predictTimeLogger(mLastTrainDuration);
  ...
  mInferReuse = infer_reuse && (idCount > 0) && (idCount == mStateSteps.size());
  if (infer_reuse) {
    for (int i = 0; i < idCount; i ++) {
