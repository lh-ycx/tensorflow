...
JNI_METHOD(void, train)
(JNIEnv* env, jobject thiz, jintArray jids, jint jwordCount, jint jletterCount, jint jid_out, jfloat jlearning_rate, jboolean jinfer_reuse) {
	LOGD("JNI_train_online");
	deeptype->train_online(env->GetIntArrayElements(jids, 0), jwordCount, jletterCount, jid_out, jlearning_rate, jinfer_reuse);
}
...
JNI_METHOD(jint, lastPredictTimeInMillis)
...
JNI_METHOD(jint, lastTrainTimeInMillis)
(JNIEnv* env, jobject thiz) {
	// LOGD("JNI_lastPredictTimeInMillis");
	return deeptype->lastTrainTimeInMillis();
}

JNI_METHOD(jboolean, lastInferReused)
(JNIEnv* env, jobject thiz) {
	// LOGD("JNI_lastPredictTimeInMillis");
	return deeptype->lastInferReused();
}

JNI_METHOD(jboolean, saveModel)
(JNIEnv* env, jobject thiz) {
	// LOGD("JNI_lastPredictTimeInMillis");
	return deeptype->saveModel();
}
